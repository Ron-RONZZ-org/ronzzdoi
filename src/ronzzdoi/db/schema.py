...
V1 = """
-- ═══════════════════════════════════════════════════════════════════
-- Version 1 — v0.1.0 initial schema
-- ═══════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS dois (
    ...
    doi_type      TEXT NOT NULL DEFAULT 'external'
                  CHECK (doi_type IN (
                      'external', 'internal',
                      'internal_circulaire', 'internal_rulebook',
                      'internal_generic', 'internal_media'
                  )),
...
-- Citation table (one DOI can have many citations)
...
CREATE VIRTUAL TABLE IF NOT EXISTS dois_fts USING fts5(
    doi, title, creator, metadata_json,
    content='dois',
    content_rowid='rowid'
);
...
AFTER INSERT ON dois BEGIN
    INSERT INTO dois_fts(rowid, doi, title, creator, metadata_json)
    VALUES (new.rowid, new.doi, new.title, new.creator, new.metadata_json);
END;

CREATE TRIGGER IF NOT EXISTS dois_fts_delete
AFTER DELETE ON dois BEGIN
    INSERT INTO dois_fts(dois_fts, rowid, doi, title, creator, metadata_json)
    VALUES ('delete', old.rowid, old.doi, old.title, old.creator, old.metadata_json);
END;
